...
router.get("/",(req,res)=>{
  ...
  if(cid===undefined||cid==0){
    ...
    var sql=`select vid, vname, vaddress, vpic, count(*) as tcount from tours inner join venues using(vid) inner join cities using (cid) where cid=? and time>=? group by vid order by tcount desc, time`;
    var params=[cid, start];
  }
  pool.query(sql,params,(err,result)=>{
    if(err){
